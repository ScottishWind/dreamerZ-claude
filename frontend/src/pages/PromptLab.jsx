...
These are all AI working behind the scenes!`,
  ...
  bestAnswer: {
    response: `# Understanding AI: A Teen's Guide 🤖

## What is AI? (The Cricket Analogy 🏏)
...
**🎯 Fun Fact:** The AI in video games that plays against you uses the same tech that helps scientists discover new medicines!`,
...
export const PromptLab = () => {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState('base'); // 'base', 'context', 'best'
  const [isLoading, setIsLoading] = useState(false);
  const [loadingType, setLoadingType] = useState(null); // 'run', 'addContext', 'improve'
  ...
  const buildPrompt = useCallback((level) => {
    ...
    switch (level) {
      ...
      case 'best':
        return `Context about me: ${context}\n\nMy question: ${goal}\n\nConstraints:\n${constraints}\n\nPlease format your response as: ${formatInstruction}`;
      default:
        return goal;
    }
  }, [goal, context, constraints, outputFormat]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950">
      {/* Safety Banner */}
      <div className="pt-16">
        <SafetyBanner variant="promptLab" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-3 bg-primary/10 px-4 py-2 rounded-full mb-4">
            ...
            <span className="text-primary font-medium">Interactive Learning</span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">Prompt Lab</h1>
          <p className="text-slate-400 max-w-2xl mx-auto">
            Learn that <span className="text-primary font-medium">better prompts = better answers</span>. 
            See how adding context and constraints transforms AI responses.
          </p>
        </motion.div>
            ...
            <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden min-h-[600px] flex flex-col">
              ...
              <div className="flex-grow p-5 overflow-auto">
                <AnimatePresence mode="wait">
                  ...
                  ) : responses[activeTab === 'context' ? 'withContext' : activeTab === 'best' ? 'bestAnswer' : 'base'] ? (
                    <motion.div
                      key={activeTab}
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className="bg-slate-900/50 rounded-xl p-5 border border-slate-700/30">
                        <div className="prose prose-invert prose-sm max-w-none">
                          <div className="text-slate-200 whitespace-pre-wrap leading-relaxed" data-testid="response-content">
                            {responses[activeTab === 'context' ? 'withContext' : activeTab === 'best' ? 'bestAnswer' : 'base']?.response}
                          </div>
                        </div>
                      </div>

                      {/* Why This Improved */}
                      {responses[activeTab === 'context' ? 'withContext' : activeTab === 'best' ? 'bestAnswer' : 'base']?.whyImproved && (
                        <motion.div
                          initial={{ opacity: 0, y: 10 }}
                          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.div
          ...
          className="mt-8 p-5 bg-slate-800/30 rounded-2xl border border-slate-700/50 flex items-start gap-4"
        >
          <AlertCircle className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
          <div className="text-sm text-slate-400">
            <strong className="text-white">Safe Learning Environment:</strong> This Prompt Lab has safety filters to keep content appropriate. 
            Never share personal information like your full name, address, phone number, or school name with any AI.
          </div>
        </motion.div>
      </div>
      </div>
